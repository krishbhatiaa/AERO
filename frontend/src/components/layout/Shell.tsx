...
import {
  ...
  LayoutDashboard, LogOut, Menu, Settings, Shield, X,
} from "lucide-react";
      ...
      <div className="flex h-screen overflow-hidden bg-background text-on-surface">
            ...
            {sidebarOpen ? (
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-on-surface-variant capitalize">{user?.role || "viewer"}</div>
                </div>
                <button onClick={handleLogout} className="rounded-md p-1.5 text-on-surface-variant hover:bg-surface-container hover:text-on-surface transition" title="Sign out">
                  <LogOut className="h-4 w-4" />
                </button>
              </div>
            ) : (
              <button onClick={handleLogout} className="flex w-full justify-center rounded-md p-2 text-on-surface-variant hover:bg-surface-container" title="Sign out">
                <LogOut className="h-4 w-4" />
              </button>
            )}
          </div>
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <aside className="absolute left-0 top-0 h-full w-64 bg-surface-container-lowest border-r border-outline-variant/40 shadow-xl">
              <div className="flex h-14 items-center justify-between border-b border-outline-variant/40 px-4">
                <div className="flex items-center gap-2">
                  <Cloud className="h-5 w-5 text-primary" />
                  <span className="text-sm font-bold">AERO-TRACK</span>
                </div>
                <button onClick={() => setMobileOpen(false)} className="rounded p-1 hover:bg-surface-container">
                  <X className="h-4 w-4" />
                </button>
              </div>
